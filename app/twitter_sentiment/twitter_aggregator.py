...
class TwitterAggregator:
	...
	def twitter_search(self, search_terms = []):
		"""
		twitter_search(self, search_terms = []):
		...
		twitter_search = twitter.Api()
		search_results = []
		results = []
		try:
			# For each search term...
			for term in search_terms:
				results.append(twitter_search.GetSearch(term))
	...
	def __get_tweet_ids(self, search_results = []):
		"""
		__get_tweet_ids( self, search_results = [] ):
				...
				for tweet in result:
					...
					for fetched_data in self.info_to_get:
						if fetched_data == 'user':
							...
							field = getattr(tweet,fetched_data)
							field = getattr(field,'screen_name')
						else:
							field = getattr(tweet,fetched_data)
						
						if (type(field) == int): 
							tweet_info.append(field)
						
						else: 
							
							tweet_info.append(field.encode('ascii', 'ignore'))
							
					# Append the information to the gathered list
					tweet_ids.append(tweet.id)
					# Put the tweet info in the database with the string ID as key
					self.redis.set(tweet.id, tweet_info)
	...
	def __is_english_tweet(self, tweet):
		...
		Determines whether a comment is an english one or not.
