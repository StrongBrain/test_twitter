...
class TwitterAnalyzer:
	...
	def __search( self ):
		...
		try:
			...
			print '2'
			self.aggregator.twitter_search(self.search_terms)
			print '3'
			print self.aggregator.twitter_search(self.search_terms)
			tweets = {} 
			for term in self.search_terms:
				tweets[term] = self.aggregator.get_tweets(q = [term], return_all_tweets = True )
			self.__end_task();
...
if __name__ == '__main__':
	search = ["#africa"]
	f = TwitterAnalyzer(search_terms = search)
	f.run()